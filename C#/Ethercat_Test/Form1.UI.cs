...
namespace Ethercat_Test
{
    ...
    public partial class Form1
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            InitAdminTab();       // 관리자 탭
            RearrangeOverview();  // 하단 재구성: Load Port 제거 + 타워/진행바 이동
        ...
        private void UpdateHmi()
        {
            ...
            towerYellow.BackColor = yellow? Color.Orange: Color.FromArgb(70, 65, 20);
        ...
        // Overview 하단 재구성: Load Port 제거, Signal Tower·챔버 진행바를 하단으로 이동.
        private void RearrangeOverview()
        {
            // Load Port 패널 숨김(미표시)
            if (pnlLoadA != null) pnlLoadA.Visible = false;
            if (pnlLoadB != null) pnlLoadB.Visible = false;

            // Signal Tower → 하단 좌측
            pnlTower.Parent = pnlBottom;
            pnlTower.Location = new Point(10, 4);
            pnlTower.Size = new Size(250, 216);

            // 챔버 진행바 → 하단 중앙 (Manual 챔버 패널에서 이동)
            AddProcRow(pbChamA, "PMC-A   PR 도포", 285, 8);
            AddProcRow(pbChamB, "PMC-B   노광", 285, 74);
            AddProcRow(pbChamC, "PMC-C   현상", 285, 140);

            // 상태 라벨 / 자동 버튼 재배치 (하단 우측)
            lblAutoStep.Location = new Point(650, 12);
            lblSlot.Location = new Point(650, 44);
            lblChamber.Location = new Point(650, 76);
            lblAlarm.Location = new Point(650, 108);
            lblAlarm.Size = new Size(560, 30);
            btnAutoStart.Location = new Point(650, 148);
            btnAutoStart.Size = new Size(180, 58);
            btnAutoStop.Location = new Point(840, 148);
            btnAutoStop.Size = new Size(180, 58);
            btnAlarmReset.Location = new Point(1030, 148);
            btnAlarmReset.Size = new Size(150, 58);
        }

        private void AddProcRow(ProgressBar pb, string caption, int x, int y)
        {
            var lbl = new Label
            {
                Text = caption,
            ...
            pb.Parent = pnlBottom;
            pb.Location = new Point(x, y + 22);
            pb.Size = new Size(330, 26);
        }
        ...
        {
            // 공정 진행 ProgressBar = 챔버 독립 공정의 (경과 / 공정시간) %
            if (pb.Style != ProgressBarStyle.Blocks) pb.Style = ProgressBarStyle.Blocks;
            int total = cham.ProcessTimeMs;
            int pct = (cham.Busy && total > 0) ? (int)((double)cham.ElapsedMs / total * 100.0) : 0;
            ...
            // 램프 라벨 = 챔버 공정상태 (Empty/Loaded/Processing/Done)
            lamp.Text = cham.ProcState.ToString();
            lamp.ForeColor = cham.Busy ? Color.OrangeRed
                : (cham.ProcState == EtherCAT_Test.Chamber.ChamberProcState.Done ? Color.SeaGreen : Color.Gray);
        }
