...
{
    // 수정 2: 챔버 자체 공정 상태 (로봇과 무관하게 스스로 진행)
    public enum ChamberProcState { Empty, Loaded, Processing, Done }
    ...
    public class Chamber
    {
        ...
        // ── 수정 2: 독립 공정 상태 ──
        public ChamberProcState ProcState { get; private set; } = ChamberProcState.Empty;
        public bool Busy => ProcState == ChamberProcState.Processing;   // 기존 필드 → 계산 프로퍼티(HMI 호환)
        private int elapsedMs;
        private int procSlot;
        public int ElapsedMs => elapsedMs;

        // 웨이퍼 안착 완료(도어 닫힘 전) → Loaded
        public void SetLoaded() { ProcState = ChamberProcState.Loaded; }

        // 도어 닫힘 확인 후 공정 시작 (로봇과 무관하게 이후 스스로 진행)
        public void StartProcess(int slot)
        {
            if (ProcState != ChamberProcState.Loaded) return;
            procSlot = slot;
            elapsedMs = 0;
            ProcState = ChamberProcState.Processing;
            LampOn();
            LogManager.Instance.WaferProcessStart(slot, Type.ToString(), Process.ToString());
        }

        // 매 스캔 호출: Processing 이면 경과 누적, ProcessTimeMs 도달 시 Done + 램프OFF + 공정완료 로그
        public void TickProcess(int intervalMs)
        {
            if (ProcState != ChamberProcState.Processing) return;
            elapsedMs += intervalMs;
            if (elapsedMs >= ProcessTimeMs)
            {
                ProcState = ChamberProcState.Done;
                LampOff();
                LogManager.Instance.WaferProcessEnd(procSlot, Type.ToString(), Process.ToString());
            }
        }

        // 로봇이 웨이퍼를 꺼낸 뒤 → Empty
        public void ClearAfterPick()
        {
            ProcState = ChamberProcState.Empty;
            HasWafer = false;
            elapsedMs = 0;
        }
